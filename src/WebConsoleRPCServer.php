    ...
    */
    ...
    private function authenticate_user($user, $password) {
        ...
        if ($user && $password) {
            ...
            if (isset($this->accounts[$user]) && !is_empty_string($this->accounts[$user])) {
                if ($this->password_hash_algorithm) $password = get_hash($this->password_hash_algorithm, $password);
    ...
    private function get_home_directory($user) {
        ...
        if (is_string($this->home_dir_conf)) {
            if (!is_empty_string($this->home_dir_conf)) return $this->home_dir_conf;
        }
        else if (is_string($user) && !is_empty_string($user) && isset($this->home_dir_conf[$user]) && !is_empty_string($this->home_dir_conf[$user]))
